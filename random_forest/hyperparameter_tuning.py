...
Optimizes on validation set, evaluates on test set.
"""
...
def tune_hyperparameters(X_train, y_train, X_val, y_val):
    ...
    Returns:
        Tuple of (best_model, best_params, grid_search_results)
    """
...
def get_top_parameters(results_df, top_n=10):
    ...
    Args:
        results_df: GridSearchCV results dataframe
    ...
    print(f"{'Rank':<5} {'n_est':<8} {'max_d':<8} {'min_spl':<8} {'min_leaf':<8} {'max_feat':<10} {'R² (mean)':<12} {'R² (std)':<10}")
    print("-" * 80)
    
    for idx, (i, row) in enumerate(top_results.iterrows(), 1):
        print(f"{idx:<5} {int(row['param_n_estimators']):<8} "
              f"{str(row['param_max_depth']):<8} "
              f"{int(row['param_min_samples_split']):<8} "
              f"{int(row['param_min_samples_leaf']):<8} "
...
def main():
    ...
    print("🍷 Wine Quality Prediction - Random Forest Hyperparameter Tuning")
    print("=" * 70)
    ...
    tuned_model, best_params, results_df = tune_hyperparameters(
    ...
    for metric in ["RMSE", "MAE", "R2"]:
        ...
        improvement_pct = (abs(improvement) / abs(baseline_val) * 100) if baseline_val != 0 else 0
        
        print(f"{metric:<20} {baseline_val:<15.4f} {tuned_val:<15.4f} {arrow} {improvement_pct:.2f}%")
    ...
    print(f"\n✅ Test R² Improvement: {test_improvement:+.4f}")
    print(f"\n🏆 Best Parameters:")
    for param, value in best_params.items():
        print(f"   {param:<25} = {value}")
    
    print(f"\n📊 Final Test Set Performance (Tuned Model):")
    print(f"   RMSE: {tuned_metrics['Test (Unseen)']['RMSE']:.4f}")
    print(f"   MAE:  {tuned_metrics['Test (Unseen)']['MAE']:.4f}")
    print(f"   R²:   {tuned_metrics['Test (Unseen)']['R2']:.4f}")
    ...
    if test_improvement > 0.05:
        ...
        print(f"\n⚠️  No improvement from tuning. Baseline may already be near-optimal.")
