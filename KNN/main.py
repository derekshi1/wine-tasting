...
from training import train_baseline, train_stepwise, train_pca, train_distance_weighted
...
def main():
    ...
    print("\n[2/3] Training stepwise feature-selection model…")
    stepwise = train_stepwise(X_train, y_train, cv=args.cv)

    print("\n[3/3] Training PCA model…")
    pca_result = train_pca(X_train, y_train, cv=args.cv)

    print("\n[4/4] Training distance-weighted KNN…")
    ...
    preds = {
        ...
        "Stepwise":  predict(stepwise),
        "PCA":       predict(pca_result),
        "Distance Weighted": predict(distance_weighted),
    }
    y_tests = {name: y_test for name in preds}
    ...
    # ── 6. Plots ──────────────────────────────────────────────────────────────
    plot_cv_k(results_map)
    plot_confusion_matrices(results_map, y_tests, preds)
